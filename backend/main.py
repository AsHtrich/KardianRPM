...
async def stream_predictions():
    return StreamingResponse(_services.prediction_stream(), media_type="text/event-stream")

@app.get("/stream_hr")
async def stream_heart_rate():
    return StreamingResponse(_services.heart_stream(), media_type="text/event-stream")

@app.get("/stream_spo2")
async def stream_spo2():
    return StreamingResponse(_services.spo2_stream(), media_type="text/event-stream")

# @app.get("/stream_predictions")
# async def stream_predictions(background_tasks: BackgroundTasks):
#     data = load_csv_data()  # Load CSV data
#     response = StreamingResponse(_services.generate_predictions(data), media_type="text/plain")
#     return response
