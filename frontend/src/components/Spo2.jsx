...
const SPO2 = () => {
    ...
    useEffect(() => {
        ...
        const startStream = () => {
            eventSource = new EventSource("http://localhost:8000/stream_spo2");  // Adjust to your backend endpoint

            eventSource.onmessage = (event) => {
                const newspo2 = JSON.parse(event.data); // Assuming the prediction is sent as JSON

                const noisyValue = newspo2.spo2;  // Add noise to the prediction value
        ...
        <div className='w-[100%] h-[280px] flex flex-row'>
            <ResponsiveContainer>
                <LineChart data={data}>
                    ...
                    />
                    <Tooltip />
                    <Line type="linear" dataKey="value" stroke="#0000FF" dot={false} />
                </LineChart>
            </ResponsiveContainer>
